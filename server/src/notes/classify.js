...
- reply — короткий ответ/уточнение/реакция на твоё же предыдущее сообщение, В ТОМ ЧИСЛЕ вопрос ПРО данные, которые ты уже показал в истории переписки (например "да", "какую?", "что из этого важно?", "а сколько там задач?" — если список уже виден в истории, отвечай по нему сам, не переспрашивай у кода) — НЕ новая мысль для сохранения.
- query — пользователь просит ПОКАЗАТЬ данные, которых ЕЩЁ НЕТ в видимой истории переписки (например "какие у меня заметки за сегодня", "что у меня на завтра", "покажи мои задачи" — когда такого списка ты ещё не показывал) — ты НЕ знаешь, что реально сохранено, поэтому НЕ придумывай ответ и НЕ сохраняй это как заметку, пусть код найдёт данные.
